...
public partial class order_category_list_item : BasePage
{
    protected int id;
    ...
    {
        Response.Redirect(Utils.CombUrlTxt("category_list_item.aspx", "keywords={0}&id={1}", txtKeywords.Text,this.id.ToString()));
    }
    /// <summary>
    /// 获得查询分页数据
    /// </summary>
    public DataSet GetList(int id, int category_id, int pageSize, int pageIndex, string strWhere, string filedOrder, out int recordCount)
    {
        string sql = "select * from FileInfo where fileType="+id+" ";
        if (strWhere.Trim() != "")
        {
            sql += " and" + strWhere;
        }
