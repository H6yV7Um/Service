...
public partial class order_order_list_going_person : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            string pageUrl = Utils.CombUrlTxt("order_list_going_person.aspx", "keywords={0}&page={1}", this.keywords, "__id__");
    ...
    protected string CombSqlTxt(string _keywords, string _property)
    {
        ...
        {
            strTemp.Append(" (title like '%" + _keywords + "%' or username like '%" + _keywords + "%' or update_name like '%" + _keywords + "%')");
        }
    ...
    {
        Response.Redirect(Utils.CombUrlTxt("order_list_going_person.aspx", "keywords={0}", txtKeywords.Text));
    }
    /// <summary>
    /// 获得查询分页数据
    /// </summary>
    ...
    {
        string sql = "select id,title,address,update_name,update_time,pingJia_time, name,telephone,username,add_time,priority,state  from dt_order where (username ='" + _userName + "' or update_name ='" + _userName + "')";
        if (strWhere.Trim() != "")
        {
            sql += " and" + strWhere;
        }
    ...
    protected void txtPageNum_TextChanged(object sender, EventArgs e)
    {
        ...
        Response.Redirect(Utils.CombUrlTxt("order_list_going_person.aspx", "", ""));
    }
    #region 返回图文每页数量=========================
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < rptList1.Items.Count; i++)
        {
            ...
            if (cb.Checked)
            {
                ...
                {
                    Response.Write("<script>alert('请确保你是报修人并且工单是“处理中”或“待确认”状态！');window.location.href ='order_list_going_person.aspx'</script>");
                    Response.End();
                }
            }
        }
        Response.Redirect("order_list_going_person.aspx");
    }
    public bool Update(int id)
    {
        ...
        if (role_id < 2 || (getManagerModel(_userName, "sms") == "1" && ("1" == getOrderModel(id.ToString(), "state") || "4" == getOrderModel(id.ToString(), "state"))) || (_userName == _firstUserName && ("1" == getOrderModel(id.ToString(), "state") || "4" == getOrderModel(id.ToString(), "state"))))
        {
    ...
    protected void btnUpdateWC_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < rptList1.Items.Count; i++)
        {
            ...
            if (cb.Checked)
            {
                ...
                Response.End();
                //Response.Write("<script>window.open('category_list.aspx');</script>");
