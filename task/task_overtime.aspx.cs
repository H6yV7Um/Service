...
public partial class task_task_overtime : BasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string strSQL = "select *,datediff(day,Task_starttime,Task_endtime)as Task_totaldays,datediff(day,Task_starttime,getdate())as Task_currentdays from T_task where (Task_empid='" + _empid + "' or receive_name='" + _empid + "') and Task_endtime<'" + date + "'and Task_handlestate=1 and Task_finishstate=0 order by Task_id desc";
        ...
        if (Rs.RowCount > 0)
        {
            ...
            {
                if (Convert.ToDateTime(Rs[i, 4]) < Convert.ToDateTime(date))
                {
